...
csv_files = [
    'testResults/measurements/mplrs_ecoli_120t_1ss/ecoli_csv.csv',
    'testResults/measurements/mplrs_ecoli_120t_2ss/ecoli_csv.csv',
    'testResults/measurements/mplrs_ecoli_60t_2ss/ecoli_csv.csv'
]
titles = ['Threads = 120, Step Size = 1', 'Threads = 120, Step Size = 2', 'Threads = 60, Step Size = 2']
...
fig.suptitle(
    r'$\mathbf{mplrs:\ Duration\ of\ Processing\ Phases\ -\ e\ coli\ core\ onto\ exchange\ reactions}$',
    fontsize=16,
...
for idx, (csv_file, title) in enumerate(zip(csv_files, titles)):
    ...
    # ax.set_yscale('log')
    ax.set_ylim(bottom=0.01, top=16)
